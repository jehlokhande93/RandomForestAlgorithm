...
class RandomForest(object):
    ...
    def _bootstrapping(self, XX, n):
        ...
        XX = np.array(XX, dtype = object) #Convert the list to a numpy array to use the default numpy functions
        
        random=np.random.randint(XX.shape[0], size=n) #randomly select n integers from 0 to XX.shape[0] and store the array in random
        
        samples = XX[random, :-1] # sampled dataset
        labels = XX[random, -1]  # class labels for the sampled records
        return (np.ndarray.tolist(samples), np.ndarray.tolist(labels)) #convert the array to list and return them
    ...
    def fitting(self):
        ...
        for i in range(self.num_trees):
            self.decision_trees[i].learn(self.bootstraps_datasets[i], self.bootstraps_labels[i]) #For each of the trees in the forest, learn the tree
    ...
    def voting(self, X):
        ...
        for record in X:
            ...
            if len(counts) == 0:
                ...
                #for i in range(self.num_trees):
                effective_vote = self.decision_trees[0].classify(record) #for each tree, and for each record, classify it
                votes.append(effective_vote)    #append the vote to the array
                counts = np.bincount(votes)     #count the number of votes
                y = np.append(y, np.argmax(counts)) 
                
            else:
                y = np.append(y, np.argmax(counts))

        return y                                #return the predicted value
...
def main():
    ...
    forest_size = 30        #Forest size chosen is 30 since the accuracy is acceptable
